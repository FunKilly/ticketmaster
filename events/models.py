...
class Event(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=EventStatusType.choices, default=EventStatusType.ACTIVE
    )
    ...
    def create_tickets(self, data):
        tickets = [
            Ticket(event=self, ticket_type=data["ticket_type"], price=data["price"])
    ...
    def available_seats(self):
        return (
            self.tickets.values("ticket_type")
            .filter(order__isnull=True)
            .annotate(available_seats=Count("ticket_type"))
        )
